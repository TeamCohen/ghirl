...
public class TextGraph implements MutableGraph, TextGraphExtensions
{
    ...
    public TextGraph(String fileStem, char mode)
    {
    	...
    	if (mode=='w') {
    		...
    		// clear the saved label and graphId files
    		rm_r(new File(idFileName));
    		rm_r(new File(labelFileName));
    ...
    public TextGraph()
    {
        ...
        try {
            ...
            System.out.println("memory-based writer = "+writer);
        } catch (IOException ex) {
    ...
    public TextGraph(String[] graphFileNames)
    {
        ...
        for (int i=0; i<graphFileNames.length; i++) {
            try {
                System.out.println("loading "+graphFileNames[i]);
    ...
    public void melt()    
    {
	    ...
	    try {
                if (indexFileName!=null) {
                    ...
                    System.out.println("opening memory-resident index for directory "+writerDirectory);
                    writer = new IndexWriter(writerDirectory, analyzer, true);
                }
	    } catch (IOException ex) {
                throw new IllegalArgumentException("can't create lucene writer: "+ex);
    ...
    public void freeze()
    {
	    ...
	    if (writer!=null) {
            ...
            if (indexFileName!=null) {
                ...
                System.out.println("opening index for memory-resident index");
                index = IndexReader.open(writerDirectory);
            }
	    searcher = new IndexSearcher(index);

            if (idFileName==null || labelFileName==null) {
                System.out.println("skipping caching of nodes and labels");
                frozenFlag = true;
                return;
            }
    ...
    private GraphId createLabelsNode(TextLabels textLabels,String labelIdShortName,GraphId textFileId)
    {
        ...
        // to the textFileId
        for (Iterator i=textLabels.getTypes().iterator(); i.hasNext(); ) {
	    ...
	    for (Span.Looper j=textLabels.instanceIterator(type); j.hasNext(); ) {
    ...
    public GraphId[] getOrderedIds()
    {
        checkFrozen();
        try {
            List idList = getListFromFile(new File(idFileName),true);
    ...
    public String[] getOrderedEdgeLabels()
    {
        checkFrozen();
        try {
	    List labelList = getListFromFile(new File(labelFileName),false);
    ...
    private static void rm_r(File f)
    {
        ...
        System.out.println("deleting: "+f+" "+(deleted?"-done":"-not present!"));
    }
