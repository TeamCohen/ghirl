...
public class PersistantCompactTokyoGraph extends AbstractCompactGraph implements
		Graph, Closable, ICompact {
	protected static final char SPACE_WORDCHAR = '\u0020';
	private static final Logger logger = Logger
			.getLogger(PersistantCompactTokyoGraph.class);
	...
	protected static final String NAME_GRAPHIDS = "_nodes",
			NAME_IDGRAPHS = "_nodeids", NAME_LINKLABELS = "_links",
			NAME_LABELLINKS = "_linkids", NAME_WALKLINKS = "_walklinks",
	...
	protected PersistantCompactTokyoGraph() {
		logger
				.warn("Note: Empty constructor for unit tests only. If this is not a unit test, try again.");
	}
	...
	public PersistantCompactTokyoGraph(String fileStem, char mode)
		...
		tc.mode = mode;
		String fqpath = Config.getProperty(Config.DBDIR) + File.separator
				+ fileStem + FILECODE + File.separator;

		File dbdir = new File(fqpath);
		if (!dbdir.exists()) dbdir.mkdir();
		int imode = tc.MODES.get(mode);
		node_id2string = tc.initDB(fqpath + NAME_GRAPHIDS, imode);
		dbs.add(node_id2string);
		node_string2id = tc.initDB(fqpath + NAME_IDGRAPHS, imode);
		dbs.add(node_string2id);
		link_id2string = tc.initDB(fqpath + NAME_LINKLABELS, imode);
		dbs.add(link_id2string);
		link_string2id = tc.initDB(fqpath + NAME_LABELLINKS, imode);
		dbs.add(link_string2id);
		walkLinks = tc.initDB(fqpath + NAME_WALKLINKS, imode);
		dbs.add(walkLinks);
		walkDistributions = tc.initDB(fqpath + NAME_WALKINFO, imode);
		dbs.add(walkDistributions);
	...
	protected void putWalkInfoLinks(int nodeid, Set<Integer> linkIds) {
		...
		try {
			...
			logger.error("Trouble closing byte array stream for writing walk links:",
					e);
		}
	...
	@Override public boolean contains(GraphId node) {
		return node_string2id.get(makeKey(node)) != null;
	}

	@Override public Iterator getNodeIterator() {
	...
	@Override public GraphId[] getOrderedIds() {
		throw new UnsupportedOperationException(
				"Can't store GraphID array in memory; use getNodeIterator() instead.");
	}

	@Override public String getProperty(GraphId from, String prop) {
		throw new UnsupportedOperationException("No properties stored on nodes in "
				+ PersistantCompactTokyoGraph.class.getName());
	}

	@Override public Distribution asQueryDistribution(String queryString) {
		return CommandLineUtil.parseNodeOrNodeSet(queryString, this);
	}

	@Override public Set followLink(GraphId from, String linkLabel) {
		Set s = new TreeSet();
	...
	@Override public Set getEdgeLabels(GraphId from) {
		...
		for (int li : getStoredWalkLinks(fromIndex)) {
			...
			if (r == null) {
				logger.debug("Unknown link in walkLinks set (node " + fromIndex
						+ ", link " + li + ")");
	...
	@Override public String getTextContent(GraphId id) {
		return id.getShortName();
	}

	@Override public Distribution walk1(GraphId from) {
	...
	protected byte[] makeKey(GraphId node) {
	...
	protected Distribution getStoredDistribution(byte[] key) {
		...
		try {
			CompactTCDistribution d = new CompactTCDistribution(result,
					node_id2string);
			return d;
		} catch (IOException e) {
			logger.error("Problem extracting distribution from disk for key " + key,
					e);
		}
	...
	protected void addWalkInfoDistribution(int fromNodeIndex, int linkIndex,
		...
		if (logger.isDebugEnabled()) {
			StringBuilder sb = new StringBuilder("Adding distribution for node "
					+ fromNodeIndex + " link " + linkIndex + " (cumulative weights):");
			for (int i = 0; i < objectIndex.length; i++)
				sb.append("\n\t" + objectIndex[i] + ":" + totalWeightSoFar[i]);
			logger.debug(sb.toString());
		}
		CompactTCDistribution ctd = new CompactTCDistribution(objectIndex,
				totalWeightSoFar, node_id2string, true);
		try {
			this.walkDistributions.put(makeKey(fromNodeIndex, linkIndex), ctd
					.serialize());
		} catch (IOException e) {
			logger.error("Problem serializing distribution for node number "
					+ fromNodeIndex + ", link number " + linkIndex, e);
		}
	}
	...
	@Override public Distribution walk1(GraphId from, String linkLabel) {
		int fromNodeIndex = getNodeIdx(from);
		if (fromNodeIndex < 0) return TreeDistribution.EMPTY_DISTRIBUTION;
	...
	@Override public GraphId[] getGraphIds() {
		throw new UnsupportedOperationException(
				"Don't even TRY to ask for an array of GraphIds!");
	}
	...
	@Override protected void initLoad(int numLinks, int numNodes) {}

	@Override protected void initLinks() {
		putLink("", 0); // add null link
	}

	@Override protected void addLink(int id, String link) {
		putLink(link, id);
	}
	...
	@Override protected void addNode(int id, String node) {
		putNode(node, id);
	}

	@Override protected void initWalkInfo() {}
	...
	@Override protected void finishWalkInfo() {
	// TODO Auto-generated method stub
	...
	public String getEdgeTypeName(int idx) {
		return getEdgeLabels().get(idx);
	}

	// used for timed graph 
	...
	public int getNodeId(String typed_name) {
		return getStoredIndex(this.node_string2id, typed_name.getBytes());
	}

	public MapID getWeightedEdges(int node, int relation) {
		return getStoredDistribution(node, relation).toMapID();
	}
